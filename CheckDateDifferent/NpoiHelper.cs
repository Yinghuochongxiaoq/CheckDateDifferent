        ...
        public DataTable ExcelToDataTable(int sheetIndex, bool isFirstRowColumn, out string errorMessage)
        {
            ...
            try
            {
                ...
                if (isFirstRowColumn)
                {
                    ...
                    {
                        var sss=firstRow.GetCell(i);
                        var column = new DataColumn(sss.StringCellValue);
                ...
                for (var i = startRow; i <= rowCount; ++i)
                {
                    ...
                    for (int j = row.FirstCellNum; j < cellCount; ++j)
                    {
                        ...
                        var str = row.GetCell(j);
                        if (str.CellType == CellType.Numeric)
                        {
                            var format = str.CellStyle.GetDataFormatString();
                            object value;
                            if (format == "m/d/yy")
                            {
                                //日期
                                value = str.DateCellValue.ToString("yyyy/M/d");
                            }
                            else if (format == "m/dd/yy")
                            {
                                //日期
                                value = str.DateCellValue.ToString("yyyy/M/dd");
                            }
                            else if (format == "mm/dd/yy")
                            {
                                //日期
                                value = str.DateCellValue.ToString("yyyy/MM/dd");
                            }
                            else if (format == "m/d/yy h:mm")
                            {
                                //时间
                                value = str.DateCellValue.ToString("yyyy/M/d H:mm:ss");
                            }
                            else if (format == "m/d/yy hh:mm")
                            {
                                //时间
                                value = str.DateCellValue.ToString("yyyy/M/d HH:mm:ss");
                            }
                            else if (format == "m/dd/yy hh:mm")
                            {
                                //时间
                                value = str.DateCellValue.ToString("yyyy/M/dd HH:mm:ss");
                            }
                            else if (format == "mm/dd/yy hh:mm")
                            {
                                //时间
                                value = str.DateCellValue.ToString("yyyy/MM/dd HH:mm:ss");
                            }
                            else if (format == "h:mm:ss")
                            {
                                //时间
                                value = str.DateCellValue.ToString("H:mm:ss");
                            }
                            else if (format == "hh:mm:ss")
                            {
                                //时间
                                value = str.DateCellValue.ToString("HH:mm:ss");
                            }
                            else
                            {
                                value = str.ToString();
        ...
        public static bool ExportExcelByDataSet(DataSet ds, string strPath, string ReportHeader = "")
        {
            ...
            for (int p = 0; p < ds.Tables.Count; p++)
            {
                ...
                for (int k = 0; k < ds.Tables[p].Columns.Count; k++)
                {
                    ...
                    mColumnName = ds.Tables[p].Columns[k].ColumnName;
                ...
                {
                    var row2 = (HSSFRow)sheet.CreateRow(i + rowCount + 1);
                    int b = 0;
                    for (int j = 0; j < ds.Tables[p].Columns.Count; j++)
                    {
                        string DgvValue = ds.Tables[p].Rows[i][j].ToString();
            ...
            //创建excel	
            var file3 = new FileStream(ReportPath, FileMode.Create);
